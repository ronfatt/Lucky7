    ...
    <div className="space-y-6 max-w-6xl mx-auto pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-2xl bg-[#0F1420]/90 border border-gold-500/20 shadow-xl">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
          {/* Main date & refresh row */}
          <div className="flex items-center gap-2">
            <input
              type="date"
              value={selectedDate}
              onChange={(e) => setSelectedDate(e.target.value)}
              className="bg-[#070A12] border border-slate-800 text-xs text-gold-300 px-3 py-1.5 rounded-xl focus:outline-none focus:border-gold-500/50 flex-1 sm:flex-initial"
            />
          ...
          {/* Secondary tools in horizontal scrollable strip on mobile */}
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 sm:pb-0 -mx-1 px-1 no-scrollbar">
            <Button
              onClick={() => setIsLedgerModalOpen(true)}
              variant="outline"
              className="px-2.5 py-1.5 rounded-xl border-slate-700 text-slate-300 hover:bg-slate-800 text-xs flex items-center gap-1 shrink-0"
            >
            ...
            <Link
              ...
              className="px-2.5 py-1.5 rounded-xl bg-slate-900 border border-slate-800 hover:border-slate-700 text-slate-300 text-xs font-medium transition flex items-center gap-1 shrink-0"
            >
              <span>深度中枢</span>
      ...
      <Card className="bg-gradient-to-br from-[#101626] via-[#0C101A] to-[#070A12] border-gold-500/50 p-5 sm:p-8 text-center relative overflow-hidden shadow-gold-glow">
        ...
        {/* Action Buttons for Number (2x2 Grid on Mobile, Flex on Tablet/Desktop) */}
        <div className="grid grid-cols-2 sm:flex sm:flex-wrap items-center justify-center gap-2 sm:gap-3 pt-2 sm:pt-3">
          <Button
            onClick={handleCopy}
            className="bg-gold-500 hover:bg-gold-400 text-obsidian-950 font-bold text-xs px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl flex items-center justify-center gap-1.5 shadow-lg shadow-gold-500/20"
          >
            {copied ? (
              <>
          ...
          <Button
            ...
            variant="outline"
            className="border-emerald-500/40 text-emerald-300 hover:bg-emerald-500/10 text-xs px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl flex items-center justify-center gap-1.5"
          >
          ...
          <Button
            ...
            variant="outline"
            className="border-sky-500/40 text-sky-300 hover:bg-sky-500/10 text-xs px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl flex items-center justify-center gap-1.5"
          >
            {isReportCopied ? (
              <>
          ...
          <Button
            onClick={handleSave}
            variant="outline"
            className="border-amber-500/40 text-amber-300 hover:bg-amber-500/10 text-xs px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl flex items-center justify-center gap-1.5"
          >
            {isSaved ? (
              <>
                <Check className="w-3.5 h-3.5 text-emerald-400" />
      ...
      <Card className="bg-[#0B0F19]/95 border-gold-500/30 p-4 sm:p-6 space-y-4 shadow-xl">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-3 border-b border-slate-800 gap-2">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400 mt-0.5">
                实时比对万能、大马彩、多多、沙巴、砂拉越、山打根及新加坡博彩开奖库
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Badge variant="gold" className="text-xs font-mono">
              累计命中: {patternAnalysis.totalHits || 0} 次
            </Badge>
        ...
        {/* Mobile Swipe Hint */}
        <div className="text-[11px] text-gold-400/80 sm:hidden flex items-center justify-between px-1">
          <span>出奖明细列表</span>
          <span>👈 左右滑动查看完整平台 👉</span>
        </div>
